...
// NOUVEAU — Ajoute champ WhatsApp + createdAt pour 48h essai
async function doRegister() {
  ...
  if (!whatsapp || whatsapp.length < 8) { err.textContent = "Numéro WhatsApp requis (ex: 0708463003)"; err.classList.add('show'); return; }
  ...
  try {
    ...
    await window._fbSetDoc(docRef, {
      company, whatsapp, compte701, exercice,
      password: btoa(pass),
      createdAt: new Date().toISOString(),
      subscriptionActive: null,
      subscriptionExpiry: null
    });
    // Sauvegarder session automatique (remember me)
    localStorage.setItem('syscohada_session', JSON.stringify({ profileId, company }));
    toast('Profil créé ! Bienvenue dans COMEO AI — 48h d\'essai gratuit.', 'success');
    switchTab('login');
    document.getElementById('l-company').value = company;
  } catch (e) { err.textContent = 'Erreur : ' + e.message; err.classList.add('show'); }
}
// NOUVEAU — Vérifie abonnement + ouvre modal paiement si expiré
const TRIAL_MS = 48 * 3600 * 1000;

function getSubStatus(profile) {
  const now = Date.now();
  if (profile.subscriptionActive && profile.subscriptionExpiry) {
    return now < profile.subscriptionExpiry ? 'active' : 'expired';
  }
  if (profile.subscriptionActive === false) return 'inactive';
  if (profile.createdAt) {
    const created = new Date(profile.createdAt).getTime();
    return (now - created) < TRIAL_MS ? 'trial' : 'expired';
  }
  return 'expired';
}
...
async function doLogin() {
  ...
  try {
    ...
    const status = getSubStatus(profile);
    if (status === 'expired' || status === 'inactive') {
      showPaymentModal(company);
      return;
    }
    await loadApp();
  } catch (e) { err.textContent = 'Erreur : ' + e.message; err.classList.add('show'); }
...
// INIT SESSION
// ══════════════════════════════════════════
// NOUVEAU — Reconnexion auto + vérification abonnement
  ...
  if (session) {
    try {
      ...
      if (snap.exists()) {
        ...
        const status = getSubStatus(currentProfile);
        if (status === 'expired' || status === 'inactive') {
          showPaymentModal(currentProfile.company);
          return;
        }
        await loadApp();
      }
...
// === À AJOUTER dans as.js ===

// Affiche le modal de paiement
function showPaymentModal(company) {
  const modal = document.getElementById('paymentModal');
  if (!modal) return;
  // Mettre à jour le lien WhatsApp avec le nom d'entreprise
  const waLink = document.getElementById('waPayLink');
  if (waLink && company) {
    waLink.href = `https://wa.me/2250508463003?text=Bonjour%2C%20je%20souhaite%20m%27abonner%20%C3%A0%20COMEO%20AI%20v4%20pour%20l%27entreprise%20${encodeURIComponent(company)}`;
  }
...
// Exposer les nouvelles fonctions globalement (ajouter à la fin de as.js)
window.showPaymentModal  = showPaymentModal;
window.hidePaymentModal  = hidePaymentModal;
window.showForgotMsg     = showForgotMsg;
window.getSubStatus      = getSubStatus;
